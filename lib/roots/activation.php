  ...
  <div class="wrap">
    ...
    <form method="post" action="options.php">
      ...
      <table class="form-table">
        ...
        <tr valign="top"><th scope="row"><?php _e('Create component reference previews?', 'roots'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Create component reference previews?', 'roots'); ?></span></legend>
              <select name="roots_theme_activation_options[create_component_reference_previews]" id="create_component_reference_previews">
                <option selected="selected" value="true"><?php echo _e('Yes', 'roots'); ?></option>
                <option value="false"><?php echo _e('No', 'roots'); ?></option>
              </select>
              <p class="description"><?php printf(__('Create a page called Home and set it to be the component reference previews', 'roots')); ?></p>
            </fieldset>
          </td>
        </tr>
...
function roots_theme_activation_action() {
  ...
  if ($roots_theme_activation_options['create_component_reference_previews'] === 'true') {
    $roots_theme_activation_options['create_component_reference_previews'] = false;

    $default_pages = [ 'Image Grid', 'Image Content Cards', 'Left Right', 'Content Grid' ];
    $existing_pages = get_pages();
    $temp = array();
    ...
    // create previews
    $components_folder = get_template_directory(). '/components';
    $check_components_folder = scandir($components_folder);
    $components_count = count($check_components_folder);

    // Loop through all components in components directory
    for ( $i = 0; $i < $components_count; $i++ ) {
      $file = $check_components_folder[$i];
      if( $file == '.' || $file == '..' ) {
        //echo nothing
      } else {
        if ( file_exists( get_template_directory() . '/components/' . $file . '/previews' ) ) {
          $this_preview_folder = get_template_directory() . '/components/' . $file . '/previews';
          $check_this_preview_folder = scandir($this_preview_folder);
          $previews_count = count($check_this_preview_folder);

          // Loop through all the previews in the component's previews directory
          for ( $x = 0; $x < $previews_count; $x++ ) {
            $preview = $check_this_preview_folder[$x];
            if ( $preview == '.' || $preview == '..' ) {
              //echo nothing
            } else {
              $component_slug = str_replace( '.php', '', $file );
              $component_slug = str_replace( '-', ' ', $component_slug );
              $component_title = ucwords($component_slug);

              $preview_slug = str_replace( 'preview-', '', $preview );
              $preview_slug = str_replace( '.php', '', $preview_slug );
              $preview_slug = str_replace( '-', ' ', $preview_slug );
              $preview_title = ucwords($preview_slug);

              // get the list of variation pages with the component as the parent
              $existing_pages = get_pages( [
                'post_type' => 'll_component',
                'parent'    => get_page_by_title( $component_title, null, 'll_component' )->ID
              ] );
              ...
              $page_exists = in_array($preview_title, $temp);

              // only create the pages that do not currently exist
              if ( !$page_exists ) {

                // only create pages for the variations
                if ( $preview !== 'preview.php' ) {
                  $new_page = array(
                    'post_title' => $preview_title,
                    'post_content' => '',
                    'post_status' => 'publish',
                    'post_type' => 'll_component',
                    'post_parent' => get_page_by_title( $component_title, null, 'll_component' )->ID
                  );

                  $result = wp_insert_post($new_page);
                }
              }
            }
          }
